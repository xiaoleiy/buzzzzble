$(document).ready(function() {
	if (Buzzzzble && Buzzzzble.DAO) {
		...
		var commentReplyArea = (function(){
			...
			var btnArea = document.createElement('ul');
			$(commentReply).append($(btnArea).addClass('btnWrapper'));
			
			var confirmBtn = document.createElement('a');
			$(confirmBtn).addClass('cbtn').append('OK');
			$(btnArea).append('<li></li>').find('li:last').append(confirmBtn);
			
			var cancelBtn = document.createElement('a');
			$(cancelBtn).addClass('cbtn').append('CANCEL');
			$(btnArea).append('<li></li>').find('li:last').append(cancelBtn);
			$(btnArea).append('<div class="clear"></div>');
			return commentReply;
		})();
		 ...
		 *     <div>
		 *     		<div class="actorAvatarWrapper">
		 *     			<img width="40" height="40" class="avatar" src=""/><br class="clear"/>		 *     			
		 *     		</div>
		 *     		<div class="contentWrapper">
		 *     			<p class="activityContent"><a class="actorName" href="">actorName</a>abcdefg</p>
		 *     		</div>
		 *     		<div class="clear"></div>
		 *     </div>
		...
		var activityItemTemplate = (function(){
			...
			$(actorAvatarWrapperSection)
					.addClass('actorAvatarWrapper')
					.append($(avatar).addClass('avatar')
			...
			$(contentWrapperSection).addClass('contentWrapper')
								.append($('<a></a>').addClass('actorName'))
								.append(source)
								.append(' -')
								.append(activityDate)
								.append($(content).addClass('activityContent'))
		 ...
		 * <li>
		 * 		<a href=""><img/></a>
		 * 		<span class="comment-date"></spam>
		 *		<p class="comment-content"><a href=""></a><br/></p>
		 *		<div class="clear"></div>
		 * </li>
		 */
		var commentItemTemplate = (function(){
			...
			$(actorAvatar).attr({'width': 34, 'height' : 34});
			...
			$(commentContent).addClass('comment-content')
							.append(commentActorName)
							.append(commentDate);
			$(commentItem).append($(avatarLink).append(actorAvatar))
		...
		var appendPhotoAttachment = function(jqPhotoListObj, photoObj){
			jqPhotoListObj.append($(photoItemTemplate).clone())
						.find('li.photoItem:last > a').attr({
		...
		var getConvertedPublishedDate = function(publishedDate){
			return new Date(publishedDate).toLocaleDateString() + ' ' + publishedDate.split('T')[1].substring(0,8);
		};
		var loadItemData = function(itemTarget, activity) {			
			...
			$(itemTarget).find('a.actorName')
				.attr('href', activity.actor.profileUrl)
				.append(activity.actor.name);
			// set activity title
			$(itemTarget).find('p.activityContent').append(activity.object.content);
			$(itemTarget).find('span.activity-source').append(' via ' + activity.source.title);
			$(itemTarget).find('span.activity-date').append(getConvertedPublishedDate(activity.published));
			//
			// load attachment
			if(activity.object.attachments){
				...
				for(var i = 0 ; i < att.length; i++ ){
					...
					switch(attItem.type){
						case 'photo':
							appendPhotoAttachment($(itemTarget).find('ul.photoList'), attItem);
							break;
					}
				}
			}			
			...
			Buzzzzble.DAO.findCommentsByActivityId(activity.id, {
				success : function(obj, code){
					...
					if(obj.data.items && obj.data.items.length > 0){
						$(itemTarget).find('span.commentNum').html(obj.data.items.length);
						...
						for(var i = 0 ; i < obj.data.items.length ; i++){
							...
							commentItem.insertBefore($(itemTarget).find('ul.commentsList > li:last'));			
						}
						// ADD OPEN LINK
						...
						if(top != bottom && top == 1){
							...
							$(templink).attr('href', '#')
								.addClass('readAllComment')
								.html(hiddenItemNum + ' older comments &#187;');
							$(templink).insertAfter($(itemTarget).find('ul.commentsList > li:first'));
							
							$(templink).click(function(){
								$(itemTarget).find('li.commentHidden').removeClass('commentHidden');
								$(templink).remove();
								return false;
							});
						}
					}else{ //  comments length equals 0
						$(itemTarget).find('div.activityCommentShortCut').css('display', 'none');			
					}					
				}
			});
		};
		var loadCommentData = function(itemTarget, comment){
			// set actor name
			$(itemTarget).find('a.actorName')
						.attr('href', comment.actor.profileUrl)
						.append(comment.actor.name);
			// set comment content
			$(itemTarget).find('p.comment-content')
						.append('<br/>')
						.append(comment.content);
			// set actor avatar
			$(itemTarget).find('img').attr('src', comment.actor.thumbnailUrl);
			// set avatar link
			$(itemTarget).find('a.avatarLink').attr('href', comment.actor.profileUrl);
			// set time
			$(itemTarget).find('span.comment-date').append(getConvertedPublishedDate(comment.published));
		};
				...
				if(textVal.length && !sending){
					...
					Buzzzzble.DAO.makeItComment(activity.id, textVal, {
						...
						success : function(comment){
							...
							/*
							 * if create successfully, animation will be occurred
							 */
							newComment.insertBefore(newItem.find('ul.commentsList > li:last'))
									.animate({
